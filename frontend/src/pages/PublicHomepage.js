...
// ];
  ...
  useEffect(() => {
    if (auth.user) {
      navigate('/home');  // Redirect to protected homepage if logged in
    }
  }, [auth, navigate]);
    ...
    const fetchTeachers = async () => {
      try {
        const res = await fetch(`${process.env.REACT_APP_API_BASE_URL}/user/teachers`);
      ...
      {teachers.map((teacher, idx) => (
        <div className="teacher-card" key={idx}>
          <div className="teacher-pic">
            <img src={teacher.profilePic || '/user_icon.png'} alt="Profile" />
          </div>
        ...
        <div className="footer-links">
          <div>
            ...
            <ul>
              ...
              style={{ cursor: 'pointer' }}>Join as Teacher</li>
            </ul>
          </div>
          <div>
            <h4>Services</h4>
